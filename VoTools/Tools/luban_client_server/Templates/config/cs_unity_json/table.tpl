...
{{~end~}}
    ...
    public bool HasVO({{cs_define_type key_type}} key) => _dataMap.ContainsKey(key);
    public {{cs_define_type value_type}} GetVO({{cs_define_type key_type}} key)
    {
        if (!_dataMap.ContainsKey(key))
            throw new Exception( $"{{name}}没有Id为{key}的记录！");
        return _dataMap[key];
    }
...
{{~end~}}
    ...
    public void _LoadData(string json_text)
    {
        JSONNode _json =  JSON.Parse(json_text);
        foreach(JSONNode _row in _json.Children)
        {
            var _v = {{cs_define_type value_type}}.Deserialize{{value_type.bean.name}}(_row);
            _dataList.Add(_v);
